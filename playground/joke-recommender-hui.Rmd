...
Jester is a joke recommender system developed at UC Berkeley to study social information filtering. Users of the system are presented a joke and then they rate them. The information is collected into two datasets that can be obtained from http://eigentaste.berkeley.edu/. One dataset is for the list jokes that the users rate. The another dataset is a collection of user's ratings.
...
The funniness of joke is very subjective.The setiment analysis is performed in this study and see if there is any relation to the joke rating and sentiment because the users require skills such as natural language processing, text analysis, computational linguistics etc to systmetically identify, extract, and quantify affective states and subjective information.For a recommender system, sentiment analysis has been proven to be a valuable technique.

##Assumptions
We assume that the users are a wide distribution of individuals from various backgrounds and geographic locations with various ages across the United States.
...
# Load the joke rating dataset and check empty values and data structure
...
# Rename columns as RATINGS and rows as userid
names(df) <- c("RATINGS",1:(ncol(df)-1))
row.names(df) <- c(1:nrow(df))

# Replace empty values as null rating 99, and replace all of 99 ratings with NA so we can filter all out
df[,2:151][df[,2:151]==""]<-99
df
df[,2:151][df[,2:151]==99]<-NA
df

# Check outliers ( outlier <- rating >10.0 or rating < -10.0)
df[,2:151]> 10.0
df[,2:151]< -10.0

# The output shows all"FALSE",inidicating there is no outliers.

```

## Load the jokes text dataset
```{r message=FALSE, warning=FALSE}
...
summary(ratingcount)
```
Anticipate that many jokes probably have low number of ratings. The fact is the median joke rating quantity is ~18% of users having rated it is quite low. 
...
```
The jokes with extremely low numbers of ratings are likely new to the system and have not been rated much.As the plot shows that there are around more than 20 less than 25 jokes has not been rated at all. Following that, there is a exponential decrease in rating count to jokes where about half of the users have rated them. The jokes where all the users rated them are "control" jokes, but it is interesting that there are no jokes between ~25k and 50k ratings.   (skew to the right)
...
```
There is an exponential drop in the number of ratings given by users. The group with ~140 ratings is potentially a control group. We will have to establish a cut off for the data that excludes users with a very low number of ratings. (skew to the right )


## Does sentiment score have an impact on the rating of a joke?
## Get sentiment score for jokes to see if there's any relation between joke rating and sentiment
...
```
The rating range in the dataset are from -10.0 to +10.0. It's interesting that substantially more jokes have a positve rating than negative. With the highest density of ratings at +3. This indicates jokes are generally enjoyed, but users don't often *love* them. 
...
qplot(rowCounts(rm), binwidth = 10, 
      ...
      ylab = "# of Jokes rated")
```
Similar logarithmic curve of ratings counts decreasing quickly. And what looks to be a control group of users with ~140 ratings.
...
```
One can see that the Popular and User-based algorithms have better true positive rate performance than random item selection. We will use the User-based CF model as it is more relevant to the assignment. 
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
#UB Pearson (User-based CF-Person)
acc_ub <- calcPredictionAccuracy(pred_ub, tst_unknown)
as(acc_ub,"matrix")

#UB Cosine (User-based CF-Cosine)
acc_ub_c <- calcPredictionAccuracy(pred_ub_c, tst_unknown)
as(acc_ub_c,"matrix")

#IB Cosine (Item-based CF-Cosine)
acc_ib <- calcPredictionAccuracy(pred_ib, tst_unknown)
as(acc_ib,"matrix")


```
The RMSE/MSE/MAE is higher than we'd like to expect for all algorithms, and is likely due to the fact that there is so much missing data. Unfortunately, we will have to continue with this model. 
We will continue with the User-Based CF with Pearson method as it has the lowest RMSE/MSE/MAE values.
...
This model is currently not of much use given its accuracy due to too much missing values, but it will serve as a proof of concept.From this model trial study, we know that the user-based CF-pearson method might produce a better outcome than other models. If possilbe, it could be used to help writers of movies/tv shows write jokes appropriate to target audiences who are looking for fun.

Definiately,more data should be collected from this userbase to fill a training dataset. The dataset in its current state is quite sparse. The data would need to be updated every 3-5 years as people's taste changes and people within certian age groups mature. The Shiny app developed would be a deployment method to collect more data. 
